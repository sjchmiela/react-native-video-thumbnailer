...
// STEP 1
// First of all, let's go through autogenerated code and understand
// what does every line do.
//
// RCT_EXPORT_MODULE() is a macro, which registers the class
// in React Native modules registry. Put it inside @implementation…@end
// in an Objective-C class to expose the class to JS. For the class
// to be instantiated and exposed properly, it has to implement
// RCTBridgeModule interface (we can verify that AJVideoThumbnailer
// does in fact implement such interface by switching to AJVideoThumbnailer.h
// file — the class declaration looks like:
// `@interface [class name] : [super class] <[implemented interfaces]>`
//
// We could provide an argument to RCT_EXPORT_MODULE macro which would then
// change the key under which the module is exposed to JS in NativeModules object.
// By default macro uses class name, so in our case RCT_EXPORT_MODULE()
// has the same effect as RCT_EXPORT_MODULE(AJVideoThumbnailer) would have.
RCT_EXPORT_MODULE()
